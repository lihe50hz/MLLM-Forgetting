...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/pasteur2/u/lihe50hz/imagenet-vqa",
    ...
    start_index = 0
    chunk_count = 0
    # Pattern to match filenames like 'train-00001-of-00044.parquet'
    ...
    # Wrap dataset in a generator to ensure it's a simple iterable for islice
    dataset_iterable = (sample for sample in dataset)
    dataset_iterator = islice(dataset_iterable, start_index, None)
    ...
    for i, (image, correct_label_idx) in progress_bar: # type: ignore
        if image.mode != "RGB":
    ...
    final_message = f"""
The dataset has been saved in {total_chunks} chunk(s) in the directory:
{output_dir}
