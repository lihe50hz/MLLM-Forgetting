...
def load_caltech_data(jsonl_path, split):
    """
    Load Caltech-101 dataset from JSONL file for a specific split.
    
    Args:
        jsonl_path (str): Path to the caltech.jsonl file
...
def load_caltech_classes(classes_path):
    """
    Load Caltech-101 class names from JSON file.
    
    Args:
        classes_path (str): Path to the caltech_classes.json file
    
    Returns:
        list: List of Caltech-101 class names
    """
    with open(classes_path, 'r', encoding='utf-8') as f:
        return json.load(f)

def main():
    parser = argparse.ArgumentParser(description="Turn Caltech-101 into a VQA dataset.")
    parser.add_argument(
        "--caltech_jsonl",
        type=str,
        default="/pasteur/u/lihe50hz/VLMTrans/data/raw_datasets/caltech.jsonl",
        help="Path to the caltech.jsonl file.",
    )
    parser.add_argument(
        "--caltech_classes",
        type=str,
        default="/pasteur/u/lihe50hz/VLMTrans/data/raw_datasets/caltech_classes.json",
        help="Path to the caltech_classes.json file.",
    )
    parser.add_argument(
        "--dataset_split",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/pasteur2/u/lihe50hz/caltech101-vqa",
    ...
    print(f"Loading Caltech-101 dataset from: {args.caltech_jsonl}")
    try:
        dataset = load_caltech_data(args.caltech_jsonl, args.dataset_split)
        print(f"Loaded {len(dataset)} samples for split '{args.dataset_split}'")
    except Exception as e:
        print(f"Failed to load Caltech-101 dataset from {args.caltech_jsonl}: {e}")
        print("Please ensure you have the caltech.jsonl file at the specified path.")
        return

    # Load Caltech-101 class names
    try:
        all_caltech_classes = load_caltech_classes(args.caltech_classes)
        print(f"Loaded {len(all_caltech_classes)} Caltech-101 classes")
    except Exception as e:
        print(f"Failed to load Caltech-101 classes from {args.caltech_classes}: {e}")
        return

    chunk_size = 1500  # Moderate chunk size for Caltech-101 dataset
    ...
    start_index = 0
    chunk_count = 0
    # Pattern to match filenames like 'test-00001-of-00003.parquet'
    ...
    # Create text prompts for all Caltech-101 classes
    text_prompts = [f"a photo of {class_name}" for class_name in all_caltech_classes]

    print("Pre-computing text embeddings for all Caltech-101 classes...")
    with torch.no_grad():
    ...
    for i, sample in progress_bar: # type: ignore
        try:
            ...
            try:
                correct_label_idx = all_caltech_classes.index(correct_label)
            except ValueError:
            ...
            for idx in sorted_indices:
                if idx.item() != correct_label_idx:
                    distractors.append(all_caltech_classes[int(idx.item())])
            ...
            if len(distractors) >= 3:
                ...
                remaining_classes = [cls for cls in all_caltech_classes if cls != correct_label]
            ...
            question = "<image>\nWhat is the object category in this image?\n"
    ...
    final_message = f"""
The Caltech-101 VQA dataset has been saved in {chunk_count} chunk(s) in the directory:
{output_dir}
