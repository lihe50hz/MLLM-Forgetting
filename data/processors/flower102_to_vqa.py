...
def main():
    parser = argparse.ArgumentParser(description="Turn Flowers-102 into a VQA dataset.")
    parser.add_argument(
        "--flowers_jsonl",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/pasteur2/u/lihe50hz/flowers102-vqa",
    ...
    chunk_size = 1000  # Smaller chunk size for flowers dataset
    ...
    start_index = 0
    chunk_count = 0
    # Pattern to match filenames like 'test-00001-of-00008.parquet'
    ...
    # Create text prompts for all flower classes
    text_prompts = [f"a photo of {flower_name}" for flower_name in all_flower_classes]

    print("Pre-computing text embeddings for all flower classes...")
    with torch.no_grad():
    ...
    for i, sample in progress_bar: # type: ignore
        try:
            ...
            question = "<image>\nWhat is the type of flower in this image?\n"
    ...
    final_message = f"""
The Flowers-102 VQA dataset has been saved in {chunk_count} chunk(s) in the directory:
{output_dir}
