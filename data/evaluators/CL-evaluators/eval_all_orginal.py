...
def eval_all(result_dir):
    ...
    for subfolder in subfolders:
        ...
        output_dir = os.path.join(result_dir, subfolder, "eval_results_original")
        if subfolder == "RS" or subfolder == "AD":
            acc = ai2d.eval_single_original(result_file, output_dir, prefix=subfolder)
        elif subfolder == "Med":
            acc = pvqa.eval_single_original(result_file, output_dir, prefix=subfolder)
        elif subfolder == "Sci":
            acc = sci.eval_single_original(result_file, output_dir, prefix=subfolder)
        elif subfolder == "Fin":
            acc = finvis.eval_single_original(result_file, output_dir, prefix=subfolder)
        else:
