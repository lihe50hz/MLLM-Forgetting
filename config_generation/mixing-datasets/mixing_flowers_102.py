...
def create_full_config_yaml(hybrid_flowers_number: float, one_minus_hybrid_flowers_number: float, folder_name: str) -> str:
...
### dataset
dataset: imagenet_vqa_train_json,flowers102_vqa_train_json
mix_strategy: interleave_over
interleave_probs: [{one_minus_hybrid_flowers_number}, {hybrid_flowers_number}]
template: qwen2_vl
cutoff_len: 16384
...
### output
output_dir: outputs/mixing-flowers/full-lowlr-sft-imagenet-vqa-{folder_name}
logging_steps: 10
save_steps: 80000
plot_loss: true
...
def create_imagenet_eval_yaml(folder_name: str, checkpoint_step: int) -> str:
    ...
    return f"""### model
model_name_or_path: outputs/mixing-flowers/full-lowlr-sft-imagenet-vqa-{folder_name}/checkpoint-{checkpoint_step}
image_max_pixels: 262144
video_max_pixels: 16384
trust_remote_code: true
...
### output
output_dir: outputs/mixing-flowers/full-lowlr-sft-imagenet-vqa-{folder_name}/checkpoint-{checkpoint_step}/eval
plot_loss: true
overwrite_output_dir: true
save_only_model: false
report_to: wandb
...
### output
output_dir: outputs/mixing-flowers/full-lowlr-sft-imagenet-vqa-{folder_name}/checkpoint-{checkpoint_step}/imagewikiqa
plot_loss: true
overwrite_output_dir: true
save_only_model: false
report_to: wandb 
...
def generate_hybrid_flowers_folder(input_number: float):
    """Generate the complete hybrid-flowers folder structure."""
    # Create folder name
    folder_name = f"hybrid-flowers-{input_number}"
    
    # Get the current directory (should be Qwen2.5-VL-3B)
    base_dir = "configs/Qwen/Qwen2.5-VL-3B/mixing-flowers"
    folder_path = os.path.join(base_dir, folder_name)
    output_path = f"outputs/mixing-flowers/full-lowlr-sft-imagenet-vqa-{folder_name}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate hybrid-flowers folder structure with configurable input number"
    )
    parser.add_argument(
        "--hybrid-ratio", 
        type=float, 
    ...
    try:
        generate_hybrid_flowers_folder(args.hybrid_ratio)
        print(f"\nSuccess! Generated hybrid-flowers-{args.hybrid_ratio} folder structure.")
