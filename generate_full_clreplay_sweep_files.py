...
def generate_result_script(scripts_dir, identifier_suffix):
    ...
    result_script_content += 'python data/evaluators/CL-evaluators/eval_all.py --result-dir working/llava-1.5-7b/full-sft-cl/Baseline\n'
    
    sorted_domains = sorted(cl_domains.items(), key=lambda x: x[1]['order'])
    
    for domain, config in sorted_domains:
        result_script_content += f'echo "-------After {domain}--------"\n'
        result_script_content += f'python data/evaluators/CL-evaluators/eval_all.py --result-dir working/llava-1.5-7b/full-sft-cl-replay-align-projector{"-" + identifier_suffix if identifier_suffix else ""}/{domain}\n'
