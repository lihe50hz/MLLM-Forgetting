...
def generate_cl_sweep(learning_rate=2.0e-5, warmup_ratio=0.03, lora_rank=128, identifier_suffix=""):
    ...
    cl_base_dir = '/pasteur/u/lihe50hz/VLMTrans/LLaVA-1.5-7B/CL-domain-sweep'
...
"""
    ...
    for i, (domain, config) in enumerate(sorted_domains):
        ...
        if i == 0:  # First domain
            ...
            model_path = f"working/llava-1.5-7b/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{prev_domain}/export"
        
        # Generate config
        content = train_template.format(
            model_path=model_path,
            dataset=config['dataset'],
            output_dir=f"working/llava-1.5-7b/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{domain}",
...
"""
    ...
    for i, (domain, config) in enumerate(sorted_domains):
        ...
        content = export_template.format(
            model_path=model_path,
            adapter_path=f"working/llava-1.5-7b/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{domain}",
            export_dir=f"working/llava-1.5-7b/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{domain}/export"
        )
...
"""
    ...
    for i, (train_domain, train_config) in enumerate(sorted_domains):
        ...
        model_path = f"working/llava-1.5-7b/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{train_domain}/export"
        ...
        for eval_domain, eval_config in cl_domains.items():
            content = eval_template.format(
                ...
                output_dir=f"working/llava-1.5-7b/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{train_domain}/{eval_domain}",
                run_name=f"llava-1.5-7b-lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}-{train_domain.lower()}-eval-{eval_domain.lower()}"
            )
