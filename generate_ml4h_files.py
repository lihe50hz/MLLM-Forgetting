...
def generate_cl_sweep(learning_rate=2.0e-5, warmup_ratio=0.03, lora_rank=128, identifier_suffix=""):
    ...
    cl_base_dir = '/pasteur/u/lihe50hz/VLMTrans/LLaVA-1.5-7B/ml4h-sweep'
...
"""
    ...
    for i, (domain, config) in enumerate(sorted_domains):
        ...
        if i == 0:  # First domain
            ...
            model_path = f"working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{prev_domain}/export"
        
        # Generate config
        content = train_template.format(
            model_path=model_path,
            dataset=config['dataset'],
            output_dir=f"working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{domain}",
            run_name=f"llava-1.5-7b-ml4h-lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}-{domain.lower()}",
            learning_rate=learning_rate,
            steps=config['steps'],
            warmup_ratio=warmup_ratio,
            lora_rank=lora_rank
...
"""
    ...
    for i, (domain, config) in enumerate(sorted_domains):
        ...
        content = export_template.format(
            model_path=model_path,
            adapter_path=f"working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{domain}",
            export_dir=f"working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{domain}/export"
        )
...
"""
    ...
    for i, (train_domain, train_config) in enumerate(sorted_domains):
        ...
        model_path = f"working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{train_domain}/export"
        ...
        for eval_domain, eval_config in cl_domains.items():
            content = eval_template.format(
                ...
                output_dir=f"working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}/{train_domain}/{eval_domain}",
                run_name=f"llava-1.5-7b-ml4h-lora-sft-cl-align-projector{'-' + identifier_suffix if identifier_suffix else ''}-{train_domain.lower()}-eval-{eval_domain.lower()}"
            )
...
def generate_result_script(scripts_dir, identifier_suffix):
    ...
    result_script_content += 'python data/evaluators/CL-evaluators/eval_all_orginal.py --result-dir working/llava-1.5-7b/ml4h/lora-sft-cl/Baseline\n'
    
    sorted_domains = sorted(cl_domains.items(), key=lambda x: x[1]['order'])
    
    for domain, config in sorted_domains:
        result_script_content += f'echo "-------After {domain}--------"\n'
        result_script_content += f'python data/evaluators/CL-evaluators/eval_all_orginal.py --result-dir working/llava-1.5-7b/ml4h/lora-sft-cl-align-projector{"-" + identifier_suffix if identifier_suffix else ""}/{domain}\n'
